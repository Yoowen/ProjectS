package me.goowen.projectm.modules.lore.tabCompleters;
...
public class LoreBookCommandTabCompleter implements TabCompleter {
    ...
    List<String> onTabComplete(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("setText")) {
            for (LoreBook loreBook : ProjectM.getLoreModule().getLoreBookList()) {
                possibleArgs.add(loreBook.getTagg());
            }
