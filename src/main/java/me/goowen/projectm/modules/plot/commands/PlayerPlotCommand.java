...
public class PlayerPlotCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "create":
                ...
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- plot with name " + tagg + " has been saved.");
                break;
            case "setowner":
                //Check if sender has the required permissions.
                ...
                if (!newOwner.hasPlayedBefore()) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "Wrong usage, player does not exist");
                    return true;
                }

                if (ProjectM.getPlotModule().getPlot(args[1]).isEmpty()) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "Wrong usage, use plot does not exist");
                    return true;
                }

                plot = ProjectM.getPlotModule().getPlot(args[1]).get();
                plot.setOwner(newOwner);
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- "+ newOwner.getName() + " has been add to the plot (Owner).");
                break;
            case "setmailbox":
                //Check if sender has the required permissions.
                ...
                plot.save();
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- Mailbox location set for " + plot.getTagg());
                break;
            case "setstatus":
                //Check if sender has the required permissions.
                if (!(sender.hasPermission("projectM.command.plot.setowner"))) {
                ...
                plot.save();
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- Plot has been set to " + plot.getPlotStatus().getPrefix());
                break;
            case "removeowner":
                //Check if sender has the required permissions.
                ...
                if (plot.getOwner() == null) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "Plot does not have an owner.");
                    return true;
                }

                plot.removeOwner(Bukkit.getOfflinePlayer(plot.getOwner()));
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- Plot owner has been removed from the plot (Owner).");
                break;
            case "addmember":
                //Check if sender has the required permissions.
                ...
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- "+ newMember.getName() + " has been added to the plot.");
                break;
            case "setshoptype":
                //Check if sender has the required permissions.
                if (!(sender.hasPermission("projectM.command.plot.setshoptype"))) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "You do not have permission to use this command!");
                    return true;
                }
                ...
                plot.setShopType(ShopType.valueOf(args[2]));
                plot.save();
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- "+ args[2] + " has been set as the shop type.");
                break;
            case "removemember":
                //Check if sender has the required permissions.
                ...
                if (plot.getMemberList().isEmpty()) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "Plot doesn't have any members.");
                    return true;
                }

                if (!plot.isMember(oldMember.getUniqueId())) {
                    sender.sendMessage(ChatColor.WHITE + "ꑜ " + ChatColor.of("#b54747") + "Wrong usage, player is not a member of this plot");
                    return true;
                }

                plot.removeMember(oldMember);
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- "+ oldMember.getName() + " has been removed from the plot.");
                break;
            case "reload":
                //Check if sender has the required permissions.
                ...
                player.sendMessage(ChatColor.of("#0ea6e9") + "Citycraft Plots" + ChatColor.WHITE + "- Plots have been reloaded.");
                break;
    ...
    public void playerHelpMessage(Player player) {
        player.sendMessage(ChatColor.WHITE + "" + ChatColor.STRIKETHROUGH + "----------------" + ChatColor.RESET + ChatColor.of("#0ea6e9")+ "" + ChatColor.BOLD + " (PLOT HELP) " + ChatColor.WHITE + "" + ChatColor.STRIKETHROUGH + "----------------");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot create " + ChatColor.GRAY + "<plotName> <worldGuardRegion> <plotType> <cost>" + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot setOwner " + ChatColor.GRAY + "<plotName> <player> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot removeOwner " + ChatColor.GRAY + "<plotName> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot addMember " + ChatColor.GRAY + "<plotName> <player> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot removeMember " + ChatColor.GRAY + "<plotName> <player> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot reload " + ChatColor.GRAY + "" + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot unavailablePlots " + ChatColor.GRAY + "<page> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot setShopType " + ChatColor.GRAY + "<plotName> <shopType> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot setMailbox " + ChatColor.GRAY + "<plotName> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
        player.sendMessage(ChatColor.of("#0ea6e9") + "/plot setStatus " + ChatColor.GRAY + "<plotName> <plotStatus> " + ChatColor.WHITE + "- Creates a new shop with the specified name.");
    ...
    public void listMessage(Integer page, Player player) {
        ...
        for (Plot value : list.subList((page - 1) * 10, maxOfPage)) {
            if (value.getPlotStatus().equals(PlotStatus.UNAVAILABLE)) {
                player.sendMessage(ChatColor.GRAY + "(" + ChatColor.UNDERLINE + value.getTagg() + ")" + ChatColor.WHITE + ", Location: " + ChatColor.ITALIC + value.getMailBoxLocation().getX() + ", " + value.getMailBoxLocation().getY() + ", " + value.getMailBoxLocation().getZ());
            }
        }
