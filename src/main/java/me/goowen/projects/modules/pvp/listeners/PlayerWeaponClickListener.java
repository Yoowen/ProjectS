...
public class PlayerWeaponClickListener implements Listener {
    ...
    public void onClick(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            GunWeapon gunWeapon = ProjectS.getPvpModule().getWeaponFromItem(itemStack);
            if (gunWeapon != null) {
                if (gunWeapon.getReloading() == 0 && !event.getPlayer().getScoreboardTags().contains("reloading")) {
                    Bukkit.getScheduler().runTaskAsynchronously(ProjectS.getInstance(), () -> gunWeapon.shoot(event.getPlayer(), event.getPlayer().getInventory().getHeldItemSlot()));
                }
            }
        }
    }
