package me.goowen.projects.framework.player;
...
public class PlayerLoader {
    ...
    public CompletableFuture<Void> savePlayer(ProjectMPlayer projectMPlayer) {
        return CompletableFuture.runAsync(() -> {
            ...
            projectS.getLog().info(ChatColor.LIGHT_PURPLE + projectMPlayer.getName() + " saved to the database!");
        });
    }
    ...
    public CompletableFuture<ProjectMPlayer> load(Player player, LoadingPlayer loadingPlayer) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (document != null && !document.isEmpty()) {
                ...
                ProjectS.getPlayerModule().getPlayersList().add(projectMPlayer);
                projectS.getLog().info(ChatColor.LIGHT_PURPLE + player.getName() + " found in Database and loaded...Welkom!");
            } else {
                projectMPlayer = new ProjectMPlayer(player.getUniqueId());
                projectMPlayer.setName(player.getName());
                savePlayer(projectMPlayer);
                ProjectS.getPlayerModule().getPlayersList().add(projectMPlayer);
                projectS.getLog().info(ChatColor.LIGHT_PURPLE + "" + player + " is new here added them in Database and loaded...Welkom!");
            }
            loadingPlayer.done(ProjectS.getPlayerModule().getPlayerDB(player));
    ...
    public CompletableFuture<ProjectMPlayer> load(OfflinePlayer player) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            ProjectMPlayer projectMPlayer;
            if (document == null || document.isEmpty()) {
                projectS.getLog().warning(ChatColor.RED+ "" + player + " Error Player was unable to load because the player does not exist!");
                return null;
            }
            projectMPlayer = gson.fromJson(document.toJson(), ProjectMPlayer.class);
            projectMPlayer.setName(player.getName());
            ProjectS.getPlayerModule().getPlayersList().add(projectMPlayer);
            projectS.getLog().info(ChatColor.LIGHT_PURPLE + player.getName() + " found in Database and loaded...");
            return projectMPlayer;
        });
    }
}